...
FLATTEN round((netPL * number(Aquired) * ClosedVal),1) as AbsPL

WHERE date(Date).month >= 6 AND
date(Date).year = 2023

SORT Date desc, Wallet desc
```

Este script de dataview, es un query que:
- buscará solo en el archivo tradelist
- buscará cualquier linea que comience con `trade::`

Y generará una tabla como la siguiente
